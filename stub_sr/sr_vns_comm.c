                         ...
                         char *server)
{
    ...
    if (strlen(sr->tmplate) > 0)
    {
        /* send VNS_OPEN_tmplate message to server */
        ot.mLen = htonl(sizeof(c_open_tmplate));
        ot.mType = htonl(VNS_OPEN_tmplate);
        strncpy(ot.tmplateName, sr->tmplate, 30);
